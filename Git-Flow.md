...
**Note 2:** Follow command line github suggestion to resolve conflicts but at the end you will not have permission to merge directly to testnet, don't push the fix to release. Create a new branch and push there, create a new pull request against `testnet` and `new_branch`, merge `new_branch` to `testnet` and `release` will be automatically added to the merge.
